...
export function StorageErrorState(props: StorageErrorStateProps) {
  return (
    <>
      {/* Error Container */}
      <main className="w-full max-w-md bg-surface-container-lowest dark:bg-inverse-surface border border-error/20 dark:border-error/30 rounded-xl p-xl shadow-[0px_4px_12px_rgba(15,23,42,0.08)] flex flex-col items-center text-center">
      {/* Error Icon */}
      ...
      <span className="material-symbols-outlined text-error text-4xl" data-icon="storage" data-weight="fill">storage</span>
      </div>
      {/* Typography */}
      ...
      <button className="w-full h-touch-target flex items-center justify-center bg-primary-container text-on-primary rounded-lg font-label-md text-label-md hover:bg-primary transition-colors focus:ring-2 focus:ring-primary focus:outline-none">
      <span className="material-symbols-outlined mr-sm text-xl" data-icon="refresh">refresh</span>
                      Retry Connection
                  </button>
      <button className="w-full h-touch-target flex items-center justify-center bg-transparent text-secondary dark:text-secondary-fixed-dim border border-outline-variant dark:border-outline/50 rounded-lg font-label-md text-label-md hover:bg-surface-container-high dark:hover:bg-surface-container transition-colors focus:ring-2 focus:ring-outline focus:outline-none">
      <span className="material-symbols-outlined mr-sm text-xl" data-icon="delete_sweep">delete_sweep</span>
                      Reset Local Data
                  </button>
      </div>
      <div className="mt-lg">
      <a className="font-label-sm text-label-sm text-primary dark:text-primary-fixed-dim hover:underline flex items-center justify-center gap-xs" href="#">
